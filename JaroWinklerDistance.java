...
public class JaroWinklerDistance{//} implements SimilarityScore<Double> {
    ...
    public Double applyCharArray(final String l, final String r) {
        final double defaultScalingFactor = 0.1;
        ...
        final int[] mtp = matchesCharArray(left, right);
        final double m = mtp[0];
        if (m == 0) {
            return 0D;
        }
    ...
    protected static int[] matchesCharArray(final char[] first, final char[] second) {
        char[] max, min;
        if (first.length > second.length) {
            max = first;
            min = second;
